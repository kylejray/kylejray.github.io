---
layout: post
title: Arbitrary Distribution Sampling II 
subtitle:  'return of the "sampling from an arbitrary distribution" '
permalink: /:title/
...
# well plot both the target and proposal distributions, for a reality check
fig, ax = plt.subplots()
...
def find_M(target, proposal, x, M_start=1, plot=True):
    # just ignore the endpoints since the probability can go to zero
    x = x[1:-1]
...
This means that we should expect the upper histogram to be more than twice as efficient at generating samples of the target distribution (and this will be the case if you test it, go ahead and give it a try.)
